...
from sarc.jobs.series import update_job_series_rgu
...
def fix_unaligned_cache(df, start, end):
...
def validate_gpu_ram(df):
    missing_ram = set(gpu_name_mapping.values()) - set(gpu_ram.keys())
    if missing_ram:
        raise ValueError(f"Missing ram: {missing_ram}")


def fix_missing_gpu_type(df, clusters):
    # Fix missing gpu_type
    for cluster_name in clusters:
        from .compute_forecast import _get_node_to_gpu

        node_to_gpu = _get_node_to_gpu(cluster_name)
        # node_to_gpu = config().clusters[cluster_name].node_to_gpu
