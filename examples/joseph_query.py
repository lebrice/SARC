...
END_DATE = datetime.datetime(2026, 4, 1, tzinfo=MTL)
...
yoshua_students: dict[str, UserData] = {}

yoshua_user = next(
    user for user in all_users if user.email == "yoshua.bengio@mila.quebec"
)

for user in all_users:
    email_to_user_or_prof[user.email] = user
    supervisors = user.supervisor.values_in_range(START_DATE, END_DATE)
    co_supervisors = user.co_supervisors.values_in_range(START_DATE, END_DATE)
    for supervisor_uuid in supervisors:
        supervisor = uuid_to_user[supervisor_uuid]
        if supervisor is yoshua_user:
            yoshua_students[user.email] = user
    for co_supervisors_at_that_time in co_supervisors:
        for co_supervisor_uuid in co_supervisors_at_that_time:
            co_supervisor = uuid_to_user[co_supervisor_uuid]
            if co_supervisor is yoshua_user:
                yoshua_students[user.email] = user

# print(yoshua_students.keys())
print(f"{len(yoshua_students)=}")
...
df = df[df["user.email"].isin(list(yoshua_students.keys()))]
...
df["supervisor.email"] = job_supervisor_emails

raw_data = df
raw_data.to_csv(f"joseph_query_raw_{date.today()}.csv")


# Group jobs by user, supervisor, and cluster.
grouped_by_user = df.query("cluster_name=='mila'").groupby(["user.email"])
...
totals = stats.sum()
# Add user percentages of the total
stats = stats.assign(
    cpu_pct=stats["cpu_cost"] / totals["cpu_cost"] * 100,
    gpu_pct=stats["gpu_cost"] / totals["gpu_cost"] * 100,
    rgu_pct=stats["rgu_cost"] / totals["rgu_cost"] * 100,
)

# Compute the total amount of compute time used, wasted and overbilled by user.
# Print from worst offender to best usage.
result = stats.sort_values(ascending=False, by="rgu_pct")
print(result.to_markdown())
result.to_csv(f"joseph_query_{date.today()}.csv")
